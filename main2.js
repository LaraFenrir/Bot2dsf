...
const serverStats = {
  guildID: '556894885404213258',
  totalUsersID: '558836513354547225',
  memberCountID: '558836533734408193',
  botCountID: '558836552852176906'
};
...
bot.on("guildMemberAdd", member => {
  ...
  member.guild.channels.find("id", "556895766304784394").send(`:white_check_mark: ${member.user.username} Est arrivé(e) ! Que son épée soit forte ! :crossed_swords: `)
...
bot.on("guildMemberRemove", member => {
  ...
  bot.channels.get(serverStats.memberCountID).setName(`Nombre de membre(s): ${member.guild.members.filter(m => !m.user.bot).size}`)
  bot.channels.get(serverStats.botCountID).setName(`Nombre de bot(s): ${member.guild.members.filter(m => m.user.bot).size}`)
  member.guild.channels.find("id", "556895801704710185").send(`:x: ${member.user.username} Est parti(e) le(la) lâche. `)
})
...
if(message.content === prefix + "neko"){
  ...
  if(message.content === prefix + "yuri"){
      ...
      superagent.get('https://nekos.life/api/v2/img/yuri')
        ...
        .setImage(response.body.url)
        .setColor(`#000000`)
        .setFooter(`Dédi à blackmoonblack car c'est le bro <3`)
        .setURL(response.body.url);
    message.channel.send({embed: lewdembed});
      })
...
if (message.content.startsWith( prefix + "roi")) {
 ...
 if(message.content === prefix + 'dev')
  ...
  .addField("Si tu lis ceci c'est pour en savoir plus sur mon développeur. Je vais tout te dire.", "Son pseudo discord est ℒ𝓪𝓻𝓪 ℱ𝒆𝓷𝓻𝓲𝓻#0666. Il est développeur depuis longtemps déjà. Il as déjà créée (avant de me créer moi) un bot musique, qui dois sans doutes être présent actuellement sur le serveur ou tu te situe (dans le cas contraire, désolée :/) essaie donc de faire '+help music' tu verras !")
